...
non-obvious connections and contradictions. The routine (filing, tagging, linking, indexing) is your job so the system stays low-maintenance.
...
- **Default to thinking mode.** Ask clarifying questions before generating. Propose;
  I dispose. Create new notes whenever necessary, but ensure they are properly labeled as generated by you.
- **When ingesting a source:** produce an atomic reading note + *proposed* concept links
  (stub missing concepts with summaries) as well as detailed summaries useful for your own crawling and linking of the content later.
...
6. **Everything in git.** Commit at logical checkpoints so mistakes are cheap.
7. **Constrain, don't sprawl.** If the structure feels inadequate, tell me and propose an
