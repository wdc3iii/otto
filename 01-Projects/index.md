---
type: index
tags: []
aliases: []
created: 2026-07-05
modified: 2026-07-05
---

# 01 · Projects

**Active outputs with a deadline or a definition of done.** One folder per output.
When a project is finished or goes dormant, move it to [[06-Archive/index|Archive]].

- `papers/` — one folder per paper (draft, notes, figures). Draws on Literature + Concepts by link; do not duplicate concept content here.
- `talks/` — one folder per talk. The `talk-prep` skill assembles material here.
- `teaching/` — per-course materials, problem sets, lectures.

Projects are the **action layer** — keep durable knowledge in
[[03-Concepts/index|Concepts]] / [[02-Literature/index|Literature]] and *link* to it,
so finished projects can be archived without losing the knowledge.
